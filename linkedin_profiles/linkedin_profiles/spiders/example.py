...
class LinkedInPeopleProfileSpider(scrapy.Spider):
    ...
    def start_requests(self):
        profile_list = [profile.strip() for profile in open("linkedin_profiles/spiders/profiles.txt").readlines()]
        print(profile_list)
    ...
    def parse_profile(self, response):
        item = {}
        file = open("linkedin_profiles/spiders/webpages/{0}.html".format(response.meta['profile']), "w")
        ...
        f = open("linkedin_profiles/spiders/data/{0}.html".format(response.meta['profile']), "w")
